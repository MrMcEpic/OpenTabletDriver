...
namespace OpenTabletDriver.Desktop.Interop.Input
{
    ...
    /// <see cref="_position"/>.
    /// </summary>
    public abstract class WindowsInkPointerBase :
        IPressureHandler,
        ITiltHandler,
        IEraserHandler,
        IMouseButtonHandler,
        ...
        IDisposable
    {
        ...
        private bool _reportEraser;
        private bool _manualEraser;
        private bool _effectiveEraser;
        private bool _prevEffectiveEraser;
        ...
        {
            _reportEraser = isEraser;
            UpdateEffectiveEraser();
        }

        public void SetManualEraser(bool active)
        {
            _manualEraser = active;
            UpdateEffectiveEraser();
        }

        public void ToggleManualEraser()
        {
            _manualEraser = !_manualEraser;
            UpdateEffectiveEraser();
        }

        private void UpdateEffectiveEraser()
        {
            var next = _reportEraser || _manualEraser;
            if (_effectiveEraser == next)
                return;

            _effectiveEraser = next;
            ...
            // goes out of range. Schedule an out-of-range teardown on the next
            // Flush so the new eraser state is picked up cleanly.
        ...
        public void Flush()
        {
            ...
            _prevEffectiveEraser = _effectiveEraser;
        }
            ...
            if (_prevInRange)
            {
                var transition = new POINTER_TYPE_INFO
                {
                    ...
                    penInfo = new POINTER_PEN_INFO
                    {
                        ...
                        penFlags = _prevEffectiveEraser ? PEN_FLAGS.INVERTED : PEN_FLAGS.NONE,
                        penMask = PEN_MASK.PRESSURE | PEN_MASK.TILT_X | PEN_MASK.TILT_Y
                    }
                };
        ...
        ~WindowsInkPointerBase() => Dispose(false);
    }
}
